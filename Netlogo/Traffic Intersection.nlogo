globals [ticks-at-last-change]  ; value of the tick counter the last time a light changed

breed [ lights light ]
breed [ accidents accident ]
breed [ cars car ]

accidents-own [clear-in]              ; how many ticks before an accident is cleared
cars-own [speed]                 ; how many patches per tick the car moves
lights-own [ver?]
patches-own [meaning]

;; ref: https://www.youtube.com/watch?v=PTOikOjSYTM
...
;
;  ask patches [
;    ifelse abs pxcor <= num-of-lanes or abs pycor <= num-of-lanes
;      [ set pcolor black ]     ; the roads are black
;      [ set pcolor green - 1 ]
;
;    if abs pxcor = num-of-lanes + 1 and abs pycor >= num-of-lanes + 1
;      [ set pcolor white ]
;    if abs pycor = num-of-lanes + 1 and abs pxcor >= num-of-lanes + 1
;     [ set pcolor white ]
;  ]
;
;  ;; bring the lights
;
;  let cnt 1
;  while [ cnt <= num-of-lanes ] [
;   ask patch cnt (-1 * num-of-lanes) [ sprout-lights 1 [ set color red set ver? true set shape "flag"] ]
;    ask patch (-1 * num-of-lanes) (-1 * cnt) [ sprout-lights 1 [ set color green set ver? false set shape "flag"] ]
;    ask patch num-of-lanes cnt [ sprout-lights 1 [ set color green set ver? false set shape "flag"] ]
;    ask patch (-1 * cnt) num-of-lanes [ sprout-lights 1 [ set color red set ver? true set shape "flag"] ]
;    set cnt cnt + 1
;  ]
...
to draw-sidewalk
  ask patches with [abs pxcor <= num-of-lanes or abs pycor <= num-of-lanes][
    set pcolor brown + 2
    sprout 1 [
...
to go
  ...
  while [ cnt <= num-of-lanes ] [
    ...
    make-new-car freq-north cnt min-pycor 0
    make-new-car freq-east min-pxcor (-1 * cnt) 90
    make-new-car freq-south (-1 * cnt) max-pycor 180
    make-new-car freq-east max-pxcor cnt 270
    set cnt cnt + 1
  ]

  let total-length ver-green-length + yellow-length + ver-red-length

  if ticks mod total-length < ver-red-length
  [ ask lights with [ver? = true][ set color red ]
    ask lights with [ver? = false][ set color green ]
  ]

  if ticks mod total-length > ver-red-length
  [ ask lights with [ver? = true][ set color green ]
    ask lights with [ver? = false][ set color red ]
  ]
