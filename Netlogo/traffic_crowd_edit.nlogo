...
to setup
  ...
  set-gis-asc
  draw-buildings
  if accident-happen? = true [ build-accident ] ;; build accident
...
to go
  ...
  ;; accident happen
  ifelse accident-happen? = true [move-people-accident][move-people]
  plot-waiting
  tick
end
...
to draw-buildings
  ;; draw the buildings
  ask patches with [(pxcor = 21 and pycor <= 36 and pycor >= 24) or
    (pxcor = 22 and pycor = 24) or (pxcor = 22 and pycor = 36) or
    (pxcor = 22 and pycor = 37) or (pxcor = 23 and pycor = 38) or
    (pxcor = 23 and pycor = 23) or (pxcor = 23 and pycor = 37) or
    (pxcor = 22 and pycor = 23) or (pxcor = 23 and pycor = 22) or
    (pxcor = 24 and pycor = 22) or (pxcor = 24 and pycor = 38) or
    (pycor = 39 and pxcor <= 56 and pxcor >= 24) or
    (pycor = 21 and pxcor <= 56 and pxcor >= 24) or
    (pxcor = 58 and pycor = 24) or (pxcor = 58 and pycor = 36) or
    (pxcor = 57 and pycor = 23) or (pxcor = 57 and pycor = 37) or
    (pxcor = 56 and pycor = 22) or (pxcor = 56 and pycor = 38) or
    (pxcor = 57 and pycor = 22) or (pxcor = 58 and pycor = 23) or
    (pxcor = 57 and pycor = 38) or (pxcor = 58 and pycor = 37) or
    (pxcor = 59 and pycor <= 36 and pycor >= 24) or (pxcor = 57 and pycor = 39) or
    (pxcor = 58 and pycor = 39) or (pxcor = 59 and pycor = 39) or
    (pxcor = 58 and pycor = 38) or (pxcor = 59 and pycor = 38) or
    (pxcor = 59 and pycor = 37)
  ][
  ...
  ;; draw the exit of big eggs
  ask patches with [(pxcor = 21 and pycor = 33) or (pxcor = 21 and pycor = 30) or (pxcor = 21 and pycor = 27) or
;    (pxcor = 20 and pycor = 33) or (pxcor = 20 and pycor = 30) or (pxcor = 20 and pycor = 27) or
    (pxcor = 24 and pycor = 38) or (pxcor = 59 and pycor = 27)
;    (pxcor = 24 and pycor = 39) or (pxcor = 60 and pycor = 27)
  ][
    set pcolor black
    set meaning "egg_exit"
    set cost 400
  ]
...
to build-accident
  ...
  let j 15 + random 10
  while [i < j][
    let ran-x 21 + random 38
    let ran-y 21 + random 18
    ask patches with [(pxcor = ran-x and pycor = ran-y) or (pxcor = ran-x + 1 and pycor = ran-y) or
      (pxcor = ran-x and pycor = ran-y + 1)  or (pxcor = ran-x + 1 and pycor = ran-y + 1)
;      or (pxcor = ran-x and pycor = ran-y + 1) or (pxcor = ran-x and pycor = ran-y - 1) or
;      (pxcor = ran-x - 1 and pycor = ran-y) or (pxcor = ran-x - 1 and pycor = ran-y + 1) or (pxcor = ran-x - 1 and pycor = ran-y - 1) or
;      (pxcor = ran-x + 1 and pycor = ran-y) or (pxcor = ran-x + 1 and pycor = ran-y + 1) or (pxcor = ran-x + 1 and pycor = ran-y - 1)
    ][
      set pcolor red
      set cost 999999999
      set meaning "accident"
    ]
    set i i + 1
  ]
end
...
to set-mrt-station
  ;; main mrt station
  ask patches with [
    (pxcor = 26 and pycor = 16) or (pxcor = 25 and pycor = 16) or
    (pxcor = 25 and pycor = 17) or (pxcor = 26 and pycor = 17) or
    (pxcor = 27 and pycor = 17) or (pxcor = 27 and pycor = 16) or
    (pxcor = 26 and pycor = 4)  or (pxcor = 25 and pycor = 4)  or
    (pxcor = 27 and pycor = 4)  or (pxcor = 25 and pycor = 3)  or
    (pxcor = 26 and pycor = 3)  or (pxcor = 27 and pycor = 3)
  ][ set meaning "mrtstation"]
end

to set-gis-asc
  ;; do it by hand in python
  set cost-dataset gis:load-dataset "geo_sim_final3.asc"
  gis:set-world-envelope gis:envelope-of cost-dataset
  gis:apply-raster cost-dataset cost

  ;; deal with no data-value
  ask patches [ifelse (cost <= 0) or (cost >= 0)[][ set cost 9999999 ]]

  ; color mrt station
  ask patches with [meaning = "mrtstation"][ set pcolor blue + 2]
  ask patches with [(pxcor = 26 and pycor = 16) or (pxcor = 26 and pycor = 4)]
  [ set exit 1 set pcolor blue]

  ;; make roads cost high
  ask patches with [(pxcor >= 6 and pxcor <= 14) or (pycor >= 5 and pycor <= 15)][ set cost 99999999999999]

end

to show_cost
  let min-cost min [cost] of patches with [cost < 9999999]
  let max-cost max [cost] of patches with [cost < 9999999]
  ask patches[ifelse cost < 9999999 [
    set pcolor scale-color blue cost max-cost min-cost
    set pcolor-back scale-color blue cost max-cost min-cost
  ][
    set pcolor 65
    set pcolor-back 65
  ]]
...
to control-traffic-lights
  if ticks mod (50 * lights-interval * greenH + 65 * lights-interval * redH ) = 0
  [change-color lightsR "H" change-color lightsL "H"]

  if ticks mod (50 * lights-interval * greenV + 65 * lights-interval * redV ) = 0
  [change-color lightsU "V" change-color lightsD "V" set lightcount lightcount + 1 crossing-light]
end

to crossing-light
  ifelse lightcount mod 2 = 0
  [
    ;; 水平
    ask patches with [ (pxcor >= 15 and pxcor <= 17 and pycor >= 5 and pycor <= 15 and meaning != "waitpoint2") or
      (pxcor >= 3 and pxcor <= 5 and pycor >= 5 and pycor <= 15 and meaning != "waitpoint2")]
    [ set meaning "crossing" ]
    ;; 垂直
    ask patches with [ (pxcor >= 6 and pxcor <= 14 and pycor >= 16 and pycor <= 18 and meaning != "waitpoint2") or
      (pxcor >= 6 and pxcor <= 14 and pycor >= 2 and pycor <= 4 and meaning != "waitpoint2")]
    [ set meaning "nocrossing" ]
  ][
    ;; 水平
    ask patches with [ (pxcor >= 15 and pxcor <= 17 and pycor >= 5 and pycor <= 15 and meaning != "waitpoint2") or
      (pxcor >= 3 and pxcor <= 5 and pycor >= 5 and pycor <= 15 and meaning != "waitpoint2")]
...
to draw-sidewalk
  ;sidewalks
  ask patches with [pxcor >= 6 and pxcor <= 14 and pycor >= 5 and pycor <= 15]
  [
    set pcolor grey
    set meaning "crossroad"
  ]

  ask patches with [meaning != "crossroad" and meaning != "mrtstation" and
    meaning != "building" and meaning != "accident" and meaning != "egg_exit"]
  [
    set pcolor brown + 2
    sprout 1 [
...
to draw-roads
  ;create crossroads
  ;; 直的：光復南路四線段，pxcor = 9為中線
  ask patches with [pxcor >= 6 and pxcor <= 9 and meaning != "crossroad"][
    set pcolor grey
    sprout 1 [
      set shape "road2"
      set color grey
   ...
   ask patches with [pxcor >= 11 and pxcor <= 14 and meaning != "crossroad"][
    set pcolor grey
    sprout 1 [
      set shape "road2"
      set color grey
  ...
  ;; 橫的：忠孝東路四段為五線段，pycor = 10為中線
  ask patches with [pycor >= 5 and pycor <= 9 and meaning != "crossroad"][
    set pcolor grey
    sprout 1 [
      set shape "road2"
      set color grey
  ...
  ask patches with [pycor >= 11 and pycor <= 15 and meaning != "crossroad"][
    set pcolor grey
    sprout 1 [
      set shape "road2"
      set color grey
  ...
  ;; the middle lanes 直的
  ask patches with [pxcor = 10 and meaning != "crossroad"] [
    set pcolor white
    sprout 1 [
  ...
  ;; the middle lanes 橫的
  ask patches with [pycor = 10 and meaning != "crossroad"] [
    set pcolor white
    sprout 1 [
      set shape "road-middle"
      set color grey - 1
...
to draw-houses&trees
  ...
  ask patches with [(meaning = "sidewalk" and pxcor >= 64)] [
    if count neighbors with [meaning = "sidewalk"] = 8 and not any? turtles in-radius 4 [
      sprout-houses 1 [
  ...
  ask patches with [(meaning = "sidewalk" and pxcor <= 18) or (meaning = "sidewalk" and pycor <= 18) ] [
    if count neighbors with [meaning = "sidewalk"] = 8 and not any? turtles in-radius 2[
      if random 100 > 85 [
        sprout-trees 1 [
...
to draw-crossings
  ...
  ;; 橫的走道
  ask patches with [(pxcor >= 6 and pxcor <= 14 and pycor >= 2 and pycor <= 4) or
  (pxcor >= 6 and pxcor <= 14 and pycor >= 16 and pycor <= 18)
  ][
    sprout-crossings 1 [
      set shape "crossing"
      set color white
  ...
  ;; 直的走道
  ask patches with [(pxcor >= 3 and pxcor <= 5 and pycor >= 5 and pycor <= 15) or
  (pxcor >= 15 and pxcor <= 17 and pycor >= 5 and pycor <= 15)]
  [
    sprout-crossings 1 [
      set shape "crossing"
      set color white
  ...
  ;; 中間的wait point
  ask crossings with [(pxcor >= 15 and pxcor <= 17 and pycor = 10) or
    (pxcor >= 3 and pxcor <= 5 and pycor = 10) or
    (pxcor = 10 and pycor >= 2 and pycor <= 4) or
    (pxcor = 10 and pycor >= 16 and pycor <= 18)]
  [
    set heading 90
    set shape "waitpoint"
...
to place-lights
  ...
  ;; place red lights in the intersection for cars
  ask patches with [(pycor >= 11 and pycor <= 15 and pxcor = 18) ][
    sprout-lightsL 1 [
      set color red
      set shape "lights"
    ]
  ...
  ask patches with [(pycor >= 5 and pycor <= 9 and pxcor = 2) ][
    sprout-lightsR 1 [
      set color red
      set shape "lights"
    ]
  ...
  ;; place green lights in the intersection for cars
  ask patches with [(pxcor >= 6 and pxcor <= 9 and pycor = 19) ][
    sprout-lightsD 1 [
      set color green
      set shape "lights"
    ]
  ...
  ask patches with [(pxcor >= 11 and pxcor <= 14 and pycor = 1) ][
    sprout-lightsU 1 [
      set color green
      set shape "lights"
    ]
  ...
  ;; set horizontal direction red lights first
  set greenH 0
  set redH 1
  set redV 0
  set greenV 1
...
to-report can-turn-right?
  ;; 可右轉道為右側第一條
  if pxcor = 14 and pycor = 4 and heading = 0 [report true]
  if pxcor = 6 and pycor = 16 and heading = 180 [report true]
  if pxcor = 15 and pycor = 15 and heading = 270 [report true]
  if pxcor = 5 and pycor = 5 and heading = 90 [report true]
  report false
end

to-report can-turn-left?
  ;; 可左轉道為左側第一條
  if pxcor = 11 and pycor = 4 and heading = 0 [report true]
  if pxcor = 9 and pycor = 16 and heading = 180 [report true]
  if pxcor = 15 and pycor = 11 and heading = 270 [report true]
  if pxcor = 5 and pycor = 9 and heading = 90 [report true]
  report false
end
...
to move-cars
  ask cars [
    ...
    if will-turn? = "maybe" [
      ...
      if turning-left? [
        if can-turn-left? [
          ifelse random 100 < prob-of-turning [
            let y-ahead 7
            if heading mod 180 = 0 [set y-ahead 9]

            move-to patch-ahead 0 lt 35
            set will-turn? "yesL"
            set turnX [pxcor] of patch-left-and-ahead 1 y-ahead  ;; angle, distance
            set turnY [pycor] of patch-left-and-ahead 1 y-ahead
    ...
    if will-turn? = "yesL" [
      ...
      [ set speed 0 ] ;; stop the car
    ]

    if meaning = "crossing" and heading mod 90 = 0 [set will-turn? "maybe"]
...
to move-people-accident
  if count persons > 0 [set move-speed count persons with [moved? = true] / count persons ]
...
to lowest_cost
  ask persons[
    set moved? false
    let target min-one-of neighbors [ cost + (count persons-here * 9999999)]

    if [cost + (count turtles-here * 9999999)] of target <= [cost] of patch-here
    [ face target
      move-to target
      set moved? true
      set wait-time 0
...
to follow_crowd
  ask persons with [moved? = false][
    ask patch-here [set p-wait p-wait + 1]
    set wait-time wait-time + 1
    let target max-one-of neighbors [ path - (count persons-here * 9999999) ]

    if [path - (count persons-here * 9999999)] of target > [path] of patch-here
    [ face target
      move-to target
      set moved? true
      set wait-time 0
...
to cross-the-street
  ...
  if crossing-part = 1[
    face min-one-of patches with [meaning = "waitpoint2"] in-radius 100 [abs([xcor] of myself - pxcor)]
...
812
671
Wait-time of pedestrians (Accident)
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if count persons > 0 [ plot mean [wait-time] of persons ]"
